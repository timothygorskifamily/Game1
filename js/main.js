(function () {
  ...
  function renderCharacterCards() {
    ...
    FamilyDash.CHARACTERS.forEach((character) => {
      ...
      card.innerHTML = `
        ...
        <p>SPD <span class="stats-row">${statBar(character.stats.speed)}</span></p>
        <p>JMP <span class="stats-row">${statBar(character.stats.jump)}</span></p>
        <p>STA <span class="stats-row">${statBar(character.stats.stamina)}</span></p>
        <p>STR <span class="stats-row">${statBar(character.stats.strength)}</span></p>
        <p>CTL <span class="stats-row">${statBar(character.stats.control)}</span></p>
      `;
  ...
  pauseToggleBtn.addEventListener("click", handlePauseToggle);

  window.addEventListener("keydown", (event) => {
    if (event.code === "KeyP" || event.code === "Escape") handlePauseToggle();
  });

  setInterval(() => {
    if (input.consumeAction("pausePressed")) handlePauseToggle();
  }, 60);
