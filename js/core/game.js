import { OBSTACLE_DEFS, POWERUP_DEFS } from "../data/levels.js";
import { intersects } from "../systems/collision.js";
import { Player } from "../entities/player.js";

export class RunnerGame {
  constructor({ canvas, input, renderer, audio, onEnd, onHud }) {
    this.canvas = canvas;
    this.input = input;
  ...
  start(character, level) {
    ...
    this.player = new Player(character, this.canvas.height, this.groundY);
  ...
  spawnObstacle() {
    const type = this.level.obstaclePool[Math.floor(Math.random() * this.level.obstaclePool.length)];
    const def = OBSTACLE_DEFS[type];
    const yBase = this.groundY - def.height;
    const y = def.flying ? yBase - 72 - Math.random() * 30 : yBase;
    this.obstacles.push({
      type,
  ...
  spawnCoin() {
    const h = 18;
    const heightJitter = Math.random() * 80;
    this.coins.push({
      x: this.canvas.width + Math.random() * 80,
      y: this.groundY - 40 - heightJitter,
      radius: h / 2,
      width: h,
      height: h
    });
  ...
  update(delta) {
    ...
    const p = this.player;
    if (this.input.consumeAction("jumpPressed")) {
      if (p.jump()) this.audio.jump();
    }
    ...
    const magnet = this.player.character.gameplay.coinMagnet || 1;
  ...
  handleCollisions() {
    ...
    this.obstacles = this.obstacles.filter((obstacle) => {
      if (!intersects(playerBox, obstacle)) return true;
      ...
      if (
        this.player.character.id === "liam" &&
        Math.random() < (this.player.character.gameplay.techShieldChance || 0)
      ) {
        return false;
      }
    ...
    this.coins = this.coins.filter((coin) => {
      if (!intersects(playerBox, coin)) return true;
      let coinValue = 1;
      if (this.player.character.id === "liam") coinValue += 1;
      if (this.player.character.id === "addy" && Math.random() < 0.1) {
        coinValue += 2;
      }
    ...
    this.powerups = this.powerups.filter((powerup) => {
      if (!intersects(playerBox, powerup)) return true;
